...
// 👉 Fetching Roles
const fetchRefundPayments = (currentPage, rowPerPage) => {
  isLoading.value = true
  const pageQueryParam = currentPage.toString();
  const params = {
    searchQuery: searchQuery.value, // Get the search query from the search box
  };
  paymentRefundRequest.fetchRefundPayments(pageQueryParam, rowPerPage, params)
    .then(response => {
      payments.value = response.data.request_data.data;
...
watchEffect(() => {
  fetchRefundPayments(paginationRef.value, rowPerPage.value);
});
...
const checkRefundStatus = async (paymentId) => {
  try {
    const response = await axios.post(`/api/web/payment/refund-status/${paymentId}`);
...
const handleAgreeClick = () => {
  checkRefundStatus(payment_id.value); // Change 'Accepted' to the desired new status
...
<template>
  <section>
    <VRow>
      ...
      <VCol cols="12">
        <VCard title="All Refund Payments">
          ...
          <VTable class="text-no-wrap"  height="27rem" fixed-header>
            ...
            <!-- 👉 table body -->
            <tbody>
              <tr v-for="(req, index) in payments" :key="req.id">
                ...
                <td>
                  <span class="text-capitalize text-base">{{ req.meta_data?.transactionId || '--' }}</span>
                ...
                <td class="text-capitalize text-high-emphasis">
                 ...
                 <td class="text-capitalize text-high-emphasis">
                  ...
                  <span class="text-base">{{ req.meta_data ? (JSON.parse(req.meta_data).amount / 100).toLocaleString('en-IN', { style: 'currency', currency: 'INR' }) : '--' }}</span>
                  <!-- <span class="text-base">{{ req.meta_data ? (JSON.parse(req.meta_data)?.amount / 100).toLocaleString('en-IN', { style: 'currency', currency: 'INR' }) : '--' }}</span> -->
                </td>
                ...
                <td class="text-center" style="inline-size: 80px;">
                  <VBtn  color="primary" small v-if="req.status == 0" @click="openConfirmationDialog(req.id)">
                    check status
                  </VBtn>
                  <span class="text-center  error"  v-else>Refund Done</span>
